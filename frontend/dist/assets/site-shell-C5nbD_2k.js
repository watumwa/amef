const v="/assets/main-logo-B6w76s6H.png",A="/assets/logo-home-final-Bl1QJRMk.png",_=document.body.dataset.page||"",g=[{id:"home",href:"/",label:"Home"},{id:"about",activePages:["about","success-story"],label:"About Us",children:[["/about/#who-we-are","Who we are"],["/about/#our-history","Our History"],["/about/#our-team","Our Team"],["/get-involved/?interest=Partnership","Partners"],["/#story","Success Stories"]]},{id:"work",activePages:["work","amhs","sifa"],label:"What we Do",children:[["/our-work/#education","Education"],["/our-work/#health-care","Health Care"],["/our-work/#livelihood","Livelihood"]]},{id:"involved",activePages:["involved","sponsor","volunteer","careers"],label:"Get Involved",children:[["/get-involved/#donate","Donate"],["/sponsor-a-child/","Sponsor a Child"],["/volunteer/","Become a Volunteer"],["/careers/","Careers"]]},{id:"news",href:"/news/",label:"News & Updates"},{id:"contact",href:"/contact/",label:"Contact Us"}],x=`
  ...
  <a class="brand brand--navbar" href="/" aria-label="AMEF home">
    <img class="brand__logo" src="${v}" alt="Asaba Memorial Education Foundation" />
  </a>`,f=e=>(e.activePages||[e.id]).includes(_),S=g.map(e=>{const a=f(e);return e.children?`
    <div class="nav-dropdown${a?" is-active":""}" data-nav-dropdown>
      ...
      <div class="nav-dropdown__menu">
        ${e.children.map(([o,t])=>`<a href="${o}">${t}</a>`).join("")}
      </div>
    </div>`:`<a class="nav-link${a?" is-active":""}" href="${e.href}"${a?' aria-current="page"':""}>${e.label}</a>`}).join(""),E=g.map(e=>e.children?`
    <details class="mobile-nav-group"${f(e)?" open":""}>
      <summary>${e.label}<svg class="icon" aria-hidden="true"><use href="#icon-chevron"></use></svg></summary>
      <div>${e.children.map(([a,o])=>`<a href="${a}">${o}</a>`).join("")}</div>
    </details>`:`<a href="${e.href}">${e.label}</a>`).join(""),b=document.querySelector("[data-site-header]");b&&(b.outerHTML=`
    <header class="site-header" data-header>
      ...
      <nav class="navbar" aria-label="Main navigation">
        ...
        <div class="mobile-menu" id="mobile-menu" data-mobile-menu><div class="container mobile-menu__inner">${E}<a class="button button--gold" href="/get-involved/#donate">Donate</a></div></div>
      </nav>
    </header>`);const p=document.querySelector("[data-site-footer]");p&&(p.outerHTML=`
    <footer class="footer">
      <div class="container footer__grid">
        <div class="footer__brand">
          <a class="brand brand--footer" href="/" aria-label="AMEF home">
            <span class="brand__logo" aria-hidden="true"><img src="${A}" alt="" /></span>
          </a>
    ...
    </footer>`);document.body.insertAdjacentHTML("afterbegin",x);const u=document.querySelector("[data-menu-toggle]"),y=document.querySelector("[data-mobile-menu]"),q=document.querySelector("[data-header]"),w=[...document.querySelectorAll("[data-nav-dropdown]")],h=e=>{u?.setAttribute("aria-expanded",String(e)),y?.classList.toggle("is-open",e),document.body.classList.toggle("menu-open",e)};u?.addEventListener("click",()=>h(u.getAttribute("aria-expanded")!=="true"));y?.querySelectorAll("a").forEach(e=>e.addEventListener("click",()=>h(!1)));const m=(e=null)=>{w.forEach(a=>{a!==e&&(a.classList.remove("is-open"),a.querySelector(".nav-dropdown__toggle")?.setAttribute("aria-expanded","false"))})};w.forEach(e=>{const a=e.querySelector(".nav-dropdown__toggle");a?.addEventListener("click",()=>{const o=!e.classList.contains("is-open");m(e),e.classList.toggle("is-open",o),a.setAttribute("aria-expanded",String(o))})});document.addEventListener("click",e=>{e.target.closest("[data-nav-dropdown]")||m()});window.addEventListener("scroll",()=>q?.classList.toggle("is-scrolled",window.scrollY>24),{passive:!0});document.addEventListener("keydown",e=>{e.key==="Escape"&&(m(),h(!1))});const k=new IntersectionObserver(e=>{e.forEach(a=>{a.isIntersecting&&(a.target.classList.add("is-visible"),k.unobserve(a.target))})},{threshold:.12,rootMargin:"0px 0px -35px"});document.querySelectorAll(".reveal").forEach(e=>k.observe(e));document.querySelectorAll("[data-current-year]").forEach(e=>{e.textContent=new Date().getFullYear()});const r=document.querySelector("[data-team-dialog]");let M=null;document.querySelectorAll("[data-team-open]").forEach(e=>{e.addEventListener("click",()=>{if(!r)return;const a=document.getElementById(e.dataset.teamOpen),o=r.querySelector("[data-team-dialog-name]"),t=r.querySelector("[data-team-dialog-role]"),n=r.querySelector("[data-team-dialog-image]"),i=r.querySelector("[data-team-dialog-biography]");!a||!o||!t||!n||!i||(o.textContent=e.dataset.teamName,t.textContent=e.dataset.teamRole,n.src=e.querySelector("img")?.currentSrc||e.querySelector("img")?.src||"",n.alt=`${e.dataset.teamName}, ${e.dataset.teamRole} at AMEF`,i.replaceChildren(a.content.cloneNode(!0)),M=e,r.showModal(),document.body.classList.add("dialog-open"))})});r?.querySelector("[data-team-dialog-close]")?.addEventListener("click",()=>r.close());r?.addEventListener("click",e=>{e.target===r&&r.close()});r?.addEventListener("close",()=>{document.body.classList.remove("dialog-open"),M?.focus()});document.querySelectorAll("[data-contact-form]").forEach(e=>{e.addEventListener("submit",a=>{a.preventDefault();const o=e.querySelector("[data-form-status]"),t=e.querySelector('button[type="submit"]'),n=new FormData(e).get("name")?.toString().trim().split(" ")[0]||"there";t.disabled=!0,t.textContent="Received",o.textContent=`Thank you, ${n}. This preview has captured your interest locally; connect the form to the client’s confirmed inbox before launch.`,o.classList.add("is-visible"),setTimeout(()=>{t.disabled=!1,t.textContent="Send another message"},1600)})});document.querySelectorAll("[data-donation-form]").forEach(e=>{const a=e.elements.customAmount,o=e.querySelector('input[name="amount"][value="other"]'),t=e.querySelector("[data-donation-error]"),n=e.querySelector("[data-donation-status]"),i=e.querySelector("[data-donation-comment-toggle]"),c=e.querySelector(".donation-comment");a?.addEventListener("focus",()=>{o.checked=!0}),a?.addEventListener("input",()=>{o.checked=!0,a.removeAttribute("aria-invalid"),t.textContent=""}),e.querySelectorAll('input[name="amount"]').forEach(l=>{l.addEventListener("change",()=>{a.removeAttribute("aria-invalid"),t.textContent="",n.classList.remove("is-visible")})}),i?.addEventListener("change",()=>{c.hidden=!i.checked,i.checked&&c.querySelector("textarea")?.focus()}),e.addEventListener("submit",l=>{l.preventDefault();const s=e.querySelector('input[name="amount"]:checked'),d=s?.value==="other"?a.value.trim():s?.value;if(!s||!d||Number(d)<=0){t.textContent="Please select or enter an amount.",s?.value==="other"?(a.setAttribute("aria-invalid","true"),a.focus()):e.querySelector('input[name="amount"]')?.focus();return}t.textContent="",n.innerHTML='Thank you for choosing to give. Online payments are not connected yet. <a href="/contact/?interest=General">Contact AMEF to complete your donation</a>.',n.classList.add("is-visible")})});document.querySelectorAll("[data-sponsorship-form]").forEach(e=>{const a=e.elements.sponsorCustomAmount,o=e.querySelector('input[name="sponsorAmount"][value="other"]'),t=e.querySelector("[data-sponsorship-error]"),n=e.querySelector("[data-sponsorship-status]"),i=e.querySelector("[data-sponsorship-comment-toggle]"),c=e.querySelector(".sponsor-now__comment");document.querySelectorAll("[data-sponsor-choice]").forEach(l=>{l.addEventListener("click",()=>{const s=e.querySelector(`input[name="sponsorFrequency"][value="${l.dataset.sponsorChoice}"]`);s&&(s.checked=!0),e.closest(".sponsor-now")?.scrollIntoView({behavior:"smooth",block:"start"}),window.setTimeout(()=>e.querySelector('input[name="sponsorAmount"]')?.focus({preventScroll:!0}),450)})}),a?.addEventListener("focus",()=>{o.checked=!0}),a?.addEventListener("input",()=>{o.checked=!0,a.removeAttribute("aria-invalid"),t.textContent=""}),e.querySelectorAll('input[name="sponsorAmount"]').forEach(l=>{l.addEventListener("change",()=>{a.removeAttribute("aria-invalid"),t.textContent="",n.classList.remove("is-visible")})}),i?.addEventListener("change",()=>{c.hidden=!i.checked,i.checked&&c.querySelector("textarea")?.focus()}),e.addEventListener("submit",l=>{l.preventDefault();const s=e.querySelector('input[name="sponsorAmount"]:checked'),d=s?.value==="other"?a.value.trim():s?.value;if(!s||!d||Number(d)<=0){t.textContent="Please select or enter an amount.",s?.value==="other"?(a.setAttribute("aria-invalid","true"),a.focus()):e.querySelector('input[name="sponsorAmount"]')?.focus();return}t.textContent="",n.innerHTML='Thank you. AMEF will confirm the sponsorship arrangement before any contribution is made. <a href="/contact/?interest=Sponsor+a+Child">Continue to the contact form</a>.',n.classList.add("is-visible")})});
