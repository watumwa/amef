const v="/assets/main-logo-B6w76s6H.png",k="/assets/logo-home-final-Bl1QJRMk.png",M=document.body.dataset.page||"",p=[{id:"home",href:"/",label:"Home"},{id:"about",label:"About Us",children:[["/about/#who-we-are","Who we are"],["/about/#our-history","Our History"],["/about/#our-team","Our Team"],["/get-involved/?interest=Partnership","Patners"],["/#story","Success Stories"]]},{id:"work",activePages:["work","amhs","sifa"],label:"What we Do",children:[["/our-work/#education","Education"],["/our-work/#health-care","Health Care"],["/our-work/#livelihood","Livelihood"]]},{id:"involved",activePages:["involved","sponsor","volunteer","careers"],label:"Get Involved",children:[["/get-involved/#donate","Donate"],["/sponsor-a-child/","Sponsor a Child"],["/volunteer/","Become a Volunteer"],["/careers/","Careers"]]},{id:"news",href:"/#news-and-updates",label:"News & Updates"},{id:"contact",href:"/contact/",label:"Contact Us"}],A=`
  ...
  <a class="brand brand--navbar" href="/" aria-label="AMEF home">
  ...
  </a>`,g=e=>(e.activePages||[e.id]).includes(M),x=p.map(e=>{const a=g(e);return e.children?`
    ...
    </div>`:`<a class="nav-link${a?" is-active":""}" href="${e.href}"${a?' aria-current="page"':""}>${e.label}</a>`}).join(""),L=p.map(e=>e.children?`
    ...
    </header>`);const b=document.querySelector("[data-site-footer]");b&&(b.outerHTML=`
    <footer class="footer">
      <div class="container footer__grid">
        <div class="footer__brand">
          <a class="brand brand--footer" href="/" aria-label="AMEF home">
            <span class="brand__logo" aria-hidden="true"><img src="${k}" alt="" /></span>
          </a>
    ...
    </footer>`);document.body.insertAdjacentHTML("afterbegin",A);const d=document.querySelector("[data-menu-toggle]"),f=document.querySelector("[data-mobile-menu]"),S=document.querySelector("[data-header]"),y=[...document.querySelectorAll("[data-nav-dropdown]")],u=e=>{d?.setAttribute("aria-expanded",String(e)),f?.classList.toggle("is-open",e),document.body.classList.toggle("menu-open",e)};d?.addEventListener("click",()=>u(d.getAttribute("aria-expanded")!=="true"));f?.querySelectorAll("a").forEach(e=>e.addEventListener("click",()=>u(!1)));const h=(e=null)=>{y.forEach(a=>{a!==e&&(a.classList.remove("is-open"),a.querySelector(".nav-dropdown__toggle")?.setAttribute("aria-expanded","false"))})};y.forEach(e=>{const a=e.querySelector(".nav-dropdown__toggle");a?.addEventListener("click",()=>{const t=!e.classList.contains("is-open");h(e),e.classList.toggle("is-open",t),a.setAttribute("aria-expanded",String(t))})});document.addEventListener("click",e=>{e.target.closest("[data-nav-dropdown]")||h()});window.addEventListener("scroll",()=>S?.classList.toggle("is-scrolled",window.scrollY>24),{passive:!0});document.addEventListener("keydown",e=>{e.key==="Escape"&&(h(),u(!1))});const w=new IntersectionObserver(e=>{e.forEach(a=>{a.isIntersecting&&(a.target.classList.add("is-visible"),w.unobserve(a.target))})},{threshold:.12,rootMargin:"0px 0px -35px"});document.querySelectorAll(".reveal").forEach(e=>w.observe(e));document.querySelectorAll("[data-current-year]").forEach(e=>{e.textContent=new Date().getFullYear()});document.querySelectorAll("[data-contact-form]").forEach(e=>{e.addEventListener("submit",a=>{a.preventDefault();const t=e.querySelector("[data-form-status]"),o=e.querySelector('button[type="submit"]'),r=new FormData(e).get("name")?.toString().trim().split(" ")[0]||"there";o.disabled=!0,o.textContent="Received",t.textContent=`Thank you, ${r}. This preview has captured your interest locally; connect the form to the client’s confirmed inbox before launch.`,t.classList.add("is-visible"),setTimeout(()=>{o.disabled=!1,o.textContent="Send another message"},1600)})});document.querySelectorAll("[data-donation-form]").forEach(e=>{const a=e.elements.customAmount,t=e.querySelector('input[name="amount"][value="other"]'),o=e.querySelector("[data-donation-error]"),r=e.querySelector("[data-donation-status]"),i=e.querySelector("[data-donation-comment-toggle]"),l=e.querySelector(".donation-comment");a?.addEventListener("focus",()=>{t.checked=!0}),a?.addEventListener("input",()=>{t.checked=!0,a.removeAttribute("aria-invalid"),o.textContent=""}),e.querySelectorAll('input[name="amount"]').forEach(s=>{s.addEventListener("change",()=>{a.removeAttribute("aria-invalid"),o.textContent="",r.classList.remove("is-visible")})}),i?.addEventListener("change",()=>{l.hidden=!i.checked,i.checked&&l.querySelector("textarea")?.focus()}),e.addEventListener("submit",s=>{s.preventDefault();const n=e.querySelector('input[name="amount"]:checked'),c=n?.value==="other"?a.value.trim():n?.value;if(!n||!c||Number(c)<=0){o.textContent="Please select or enter an amount.",n?.value==="other"?(a.setAttribute("aria-invalid","true"),a.focus()):e.querySelector('input[name="amount"]')?.focus();return}o.textContent="",r.innerHTML='Thank you for choosing to give. Online payments are not connected yet. <a href="/contact/?interest=General">Contact AMEF to complete your donation</a>.',r.classList.add("is-visible")})});document.querySelectorAll("[data-sponsorship-form]").forEach(e=>{const a=e.elements.sponsorCustomAmount,t=e.querySelector('input[name="sponsorAmount"][value="other"]'),o=e.querySelector("[data-sponsorship-error]"),r=e.querySelector("[data-sponsorship-status]"),i=e.querySelector("[data-sponsorship-comment-toggle]"),l=e.querySelector(".sponsor-now__comment");document.querySelectorAll("[data-sponsor-choice]").forEach(s=>{s.addEventListener("click",()=>{const n=e.querySelector(`input[name="sponsorFrequency"][value="${s.dataset.sponsorChoice}"]`);n&&(n.checked=!0),e.closest(".sponsor-now")?.scrollIntoView({behavior:"smooth",block:"start"}),window.setTimeout(()=>e.querySelector('input[name="sponsorAmount"]')?.focus({preventScroll:!0}),450)})}),a?.addEventListener("focus",()=>{t.checked=!0}),a?.addEventListener("input",()=>{t.checked=!0,a.removeAttribute("aria-invalid"),o.textContent=""}),e.querySelectorAll('input[name="sponsorAmount"]').forEach(s=>{s.addEventListener("change",()=>{a.removeAttribute("aria-invalid"),o.textContent="",r.classList.remove("is-visible")})}),i?.addEventListener("change",()=>{l.hidden=!i.checked,i.checked&&l.querySelector("textarea")?.focus()}),e.addEventListener("submit",s=>{s.preventDefault();const n=e.querySelector('input[name="sponsorAmount"]:checked'),c=n?.value==="other"?a.value.trim():n?.value;if(!n||!c||Number(c)<=0){o.textContent="Please select or enter an amount.",n?.value==="other"?(a.setAttribute("aria-invalid","true"),a.focus()):e.querySelector('input[name="sponsorAmount"]')?.focus();return}o.textContent="",r.innerHTML='Thank you. AMEF will confirm the sponsorship arrangement before any contribution is made. <a href="/contact/?interest=Sponsor+a+Child">Continue to the contact form</a>.',r.classList.add("is-visible")})});
