const m=document.body.dataset.page||"",c=[{id:"home",href:"/",label:"Home"},{id:"about",label:"About Us",children:[["/about/#who-we-are","Who we are"],["/about/#our-history","Our History"],["/about/#our-team","Our Team"],["/get-involved/?interest=Partnership","Patners"],["/#story","Success Stories"]]},{id:"work",activePages:["work","amhs","sifa"],label:"What we Do",children:[["/our-work/#education","Education"],["/our-work/#health-care","Health Care"],["/our-work/#livelihood","Livelihood"]]},{id:"involved",label:"Get Involved",children:[["/get-involved/#donate","Donate"],["/contact/?interest=Sponsor+a+Child","Sponsor a Child"],["/contact/?interest=Volunteer","Become a Volunteer"],["/contact/?interest=Careers","Careers"]]},{id:"news",href:"/#news-and-updates",label:"News & Updates"},{id:"contact",href:"/contact/",label:"Contact Us"}],g=`
  ...
  <a class="brand brand--navbar" href="/" aria-label="AMEF home">
    ...
    <div class="nav-dropdown${a?" is-active":""}" data-nav-dropdown>
      ...
      <div class="nav-dropdown__menu">
        ${e.children.map(([o,i])=>`<a href="${o}">${i}</a>`).join("")}
      </div>
    </div>`:`<a class="nav-link${a?" is-active":""}" href="${e.href}"${a?' aria-current="page"':""}>${e.label}</a>`}).join(""),y=c.map(e=>e.children?`
    <details class="mobile-nav-group"${h(e)?" open":""}>
    ...
    </details>`:`<a href="${e.href}">${e.label}</a>`).join(""),k=`
  <div class="footer-social">
  ...
  </div>`,r=document.querySelector("[data-site-header]");r&&(r.outerHTML=`
    <header class="site-header" data-header>
      ...
      <nav class="navbar" aria-label="Main navigation">
        ...
        <div class="mobile-menu" id="mobile-menu" data-mobile-menu><div class="container mobile-menu__inner">${y}<a class="button button--gold" href="/get-involved/#donate">Donate</a></div></div>
      </nav>
    </header>`);const l=document.querySelector("[data-site-footer]");l&&(l.outerHTML=`
    <footer class="footer">
      <div class="container footer__grid">
        <div class="footer__brand">${d(!0)}<p>Turning remembrance into action, hopelessness into hope, and potential into opportunity.</p>${k}</div>
    ...
    </footer>`);document.body.insertAdjacentHTML("afterbegin",g);const s=document.querySelector("[data-menu-toggle]"),u=document.querySelector("[data-mobile-menu]"),w=document.querySelector("[data-header]"),v=[...document.querySelectorAll("[data-nav-dropdown]")],n=e=>{s?.setAttribute("aria-expanded",String(e)),u?.classList.toggle("is-open",e),document.body.classList.toggle("menu-open",e)};s?.addEventListener("click",()=>n(s.getAttribute("aria-expanded")!=="true"));u?.querySelectorAll("a").forEach(e=>e.addEventListener("click",()=>n(!1)));const t=(e=null)=>{v.forEach(a=>{a!==e&&(a.classList.remove("is-open"),a.querySelector(".nav-dropdown__toggle")?.setAttribute("aria-expanded","false"))})};v.forEach(e=>{const a=e.querySelector(".nav-dropdown__toggle");a?.addEventListener("click",()=>{const o=!e.classList.contains("is-open");t(e),e.classList.toggle("is-open",o),a.setAttribute("aria-expanded",String(o))})});document.addEventListener("click",e=>{e.target.closest("[data-nav-dropdown]")||t()});window.addEventListener("scroll",()=>w?.classList.toggle("is-scrolled",window.scrollY>24),{passive:!0});document.addEventListener("keydown",e=>{e.key==="Escape"&&(t(),n(!1))});const p=new IntersectionObserver(e=>{e.forEach(a=>{a.isIntersecting&&(a.target.classList.add("is-visible"),p.unobserve(a.target))})},{threshold:.12,rootMargin:"0px 0px -35px"});document.querySelectorAll(".reveal").forEach(e=>p.observe(e));document.querySelectorAll("[data-current-year]").forEach(e=>{e.textContent=new Date().getFullYear()});document.querySelectorAll("[data-contact-form]").forEach(e=>{e.addEventListener("submit",a=>{a.preventDefault();const o=e.querySelector("[data-form-status]"),i=e.querySelector('button[type="submit"]'),b=new FormData(e).get("name")?.toString().trim().split(" ")[0]||"there";i.disabled=!0,i.textContent="Received",o.textContent=`Thank you, ${b}. This preview has captured your interest locally; connect the form to the client’s confirmed inbox before launch.`,o.classList.add("is-visible"),setTimeout(()=>{i.disabled=!1,i.textContent="Send another message"},1600)})});
